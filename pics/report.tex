...
\begin{itemize}
\item	Implement a curriculum that takes into account the demands of industrial artists, home planners and architectural engineers when they designed 2D floor plans and want to see them in 3D before actual implementation.
...
\item	To analyze working different softwares which make 3D models via importing floor plans in different formats as input with high precision and less effort. So, we could research on newly deployed softwares in market.
...
\begin{itemize}
\item	To ensure, that we are implementing our project by using latest technologies which helps students and technical persons to enhance their academic skills via learning and integrating new textures about 3D modelling.
\item To implement a 3D project in a way that is compatible with existing systems and capable of taking automated decision while generating 3D model.
\item To make a user friendly GUI (Graphical User Interface) that does not require too much interfaces to access 3D model generated according to the 2D floor plan.
...
\\
\hline
Free CAD Software&Jürgen Riegel, Werner Mayer, Yorik van Havre&2D drawing, 3D Modelling, Panoramic 360 Views, Parrametric Model\\
\hline
...
Operational feasibility means whether a proposed system is to be feasible at operational level. For this purpose, we conduct a market survey in which we ask many questions from common users, architects, home planners and interior designers. 71.1\% people think that their home is not according to what they have imagined at the time of constructing house(see Figure \ref{fig:one}). 87.5\% people like to do interior designing of their home on our software to see how it looks before actually doing in real environment(see Figure \ref{fig:two}). 35\% people use already existing such systems but they don’t like these systems because of hard to use and time taking steps required to generate 3D model(see Figure \ref{fig:three}). The proposed system is generate efficiently 3D model on just a button click.
\\
\\
We also conduct survey for architects, home planners and real estate seller agents. 75\% people think that they face difficulty in satisfying the customer with their floor plan designs(see Figure \ref{fig:four}). 87\% people think that the existing systems are not easy to use(see Figure \ref{fig:five}). 73\% people that existing systems are not cost effective because these softwares are not free(see Figure \ref{fig:six}). 90.9\% people are willing to use our proposed system because our proposed system automatically generate 3D model instead of make the 3D model after a lot of manual steps(see Figure \ref{fig:seven}). 
...
\caption{Survey reviews}
\end{figure}

\begin{figure}[h!]
\centering
\includegraphics[scale=0.7]{graph3}
\caption{Not desired house}
\label{fig:one}
\end{figure}
...
\caption{Interior designing of 3D model}
\label{fig:two}
\end{figure}
...
\caption{For architects and home planners}
\label{fig:four}
\end{figure}
...
\caption{Willing to use our proposed system}
\label{fig:seven}
\end{figure}
